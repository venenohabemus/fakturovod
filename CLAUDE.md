...
## 11. Lokálne vývojové prostredie (Windows, stav 07/2026)
...
- Zapnuté PHP rozšírenia: curl, fileinfo, gd, intl, mbstring, openssl, pdo_mysql, pdo_pgsql, pdo_sqlite, sqlite3, zip.
- Framework: Laravel 13 (skeleton v13.8.0, framework v13.20.0). Lokálna DB zatiaľ SQLite (`database/database.sqlite`) — na PostgreSQL prejsť, keď začne dávať zmysel (pipeline + JSONB).
- Docker ani WSL na stroji nie sú — KoSIT validator (Java sidecar) bude treba riešiť neskôr (inštalácia Javy alebo Docker Desktop).
- Testy: `php artisan test`.

## 12. Zásady vývoja (pre Claude Code)
